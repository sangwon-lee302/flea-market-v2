...
`.github/pull_request_template.md` は人間が書く pull request のためのものであり、Claude は使わない。Claude はこの節の規約に従って本文を書き、クォートしたヒアドキュメント（`<<'EOF'`）経由で `gh pr create --body` に渡す。本文には識別子のバッククォートが含まれるため、クォートしないとシェルがコマンドとして実行してしまう。テンプレートや CLAUDE.md をそのまま本文として渡さず、テンプレートを直接編集もしない。
